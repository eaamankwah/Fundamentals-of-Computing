...
def score(hand):
    ...
    for dice in hand:
        present_max_val = 0
        for present_dice in hand:
            if (present_dice == dice):
                present_max_val += dice
        if present_max_val > max_val:
            max_val = present_max_val
    return max_val
...
def gen_all_holds(hand):
    ...
    for hold in answer_set:
        ...
        counter = 0
        for value in hold:
            if hold.counter(value) > hand.counter(value):
                counter = counter + 1
        if counter == 0:
